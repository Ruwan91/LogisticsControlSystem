...
import edu.sliit.logistics_control_system.dbaccess.CustomerAccess;
...
public class GetAllCustomers extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            try {
                allCustomers = ca.getAllCustomers();
            } catch (ClassNotFoundException | SQLException ex) {
                Logger.getLogger(SearchAllDriverNamesForAddDelivery.class.getName()).log(Level.SEVERE, null, ex);
            }
            if (allCustomers != null) {
                out.print("<select class=\"form-control\" id=\"customerid\" name=\"customerid\" >");
                out.print("<option value=\"none\" id=\"selected_customerid\">");
                out.print("Select Customer");
                out.print("</option>");
                for (Customer customer : allCustomers) {
                    out.print("<option>");
                    out.print(customer.getFirstName()+" "+ customer.getLastName() + "-" + customer.getCustid());
                    out.print("</option>");
                }
                out.print("</select>");
            }
